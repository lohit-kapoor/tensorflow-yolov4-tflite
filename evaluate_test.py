...
flags.DEFINE_string('weights', '/content/tensorflow-yolov4-tflite/checkpoints_m/yolov4',
...
flags.DEFINE_string('annotation_path', "/content/tensorflow-yolov4-tflite/Combined_F/annotations.txt", 'annotation path')
flags.DEFINE_string('write_image_path', "/content/tensorflow-yolov4-tflite/data/detection_F/", 'write image path')
        ...
        for num, line in enumerate(annotation_file):
            ...
            with open(predict_result_path, 'w') as f:
                ...
                for i in range(valid_detections[0]):
                    ...
                    coor = boxes[0][i]
                    coor_ = np.copy(coor)
                    coor_[0] = int(coor[0] * image_h)
                    coor_[2] = int(coor[2] * image_h)
                    coor_[1] = int(coor[1] * image_w)
                    coor_[3] = int(coor[3] * image_w)

                    score = scores[0][i]
                    class_ind = int(classes[0][i])
                    class_name = CLASSES[class_ind]
                    score = '%.4f' % score
                    ymin, xmin, ymax, xmax = list(map(str, coor_))
